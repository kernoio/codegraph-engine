...
export const expressResolver: FrameworkResolver = {
  ...
  detect(context: ResolutionContext): boolean {
    ...
    // Fastify / Koa / Hapi / Adonis are owned by dedicated Kerno plugins, so we
    // only claim `express` itself here (unlike upstream's broader dependsOn list).
    if (dependsOn(context, 'express')) return true;
